    ...
    return (
        <div className={classes.contentLinkOn}>
            {repos.map((repo, index) => {
                return (index + 1 === 1 ?
                    <Link className={pathname === '/' ? classes.myLink : classes.myLinkOn} key={index} to={'/'}>{index + 1}</Link> :
                    <Link className={Number(page) === index + 1 ? classes.myLink : classes.myLinkOn} key={index} to={`/page/${index + 1}`}>{index + 1}</Link>
                )
            })}
        </div>
    );
